...
def run_simulation(nx, ny, run_index):
    """Run the cubic3lay simulation for a given lattice size"""
    print(f"Running simulation {run_index+1}/3 for lattice size {nx}x{ny}...")
...
def average_simulations(nx, ny, num_runs=3):
    """Run multiple simulations and average the results"""
    all_runs_data = []
    
    for i in range(num_runs):
        data = run_simulation(nx, ny, i)
        if data is None:
            print(f"Failed to complete run {i+1}")
            return None
...
def main():
    ...
    print(f"Running {nx}x{ny} simulation with 3 runs for averaging")
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Run simulations and average results
    avg_data = average_simulations(nx, ny, num_runs=3)
    if avg_data is None:
