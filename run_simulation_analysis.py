...
# Theoretical values for 8-state Potts model
THEORETICAL_TC = 1.0 / np.log(1 + np.sqrt(8))  # ~0.751
...
def run_simulation(nx, ny):
    ...
    input_str = f"{nx} {ny} {MONTE_CARLO_STEPS_EQUILIBRATION} {MONTE_CARLO_STEPS_MEASUREMENT} {RANDOM_SEED}"
    ...
    print(f"Simulation for {nx}x{ny} completed successfully")
    return True
...
def critical_exponents_analysis(all_data):
    ...
    for size, data in all_data.items():
        ...
        chi_vals = size*size * m_vals / t_vals[idx]
...
def main():
    ...
    print("Starting comprehensive simulation and analysis...")
    ...
    if need_simulations:
        print("Some simulations need to be run...")
        all_data = {}
